...
OUT=ROOT.parent/'verification';OUT.mkdir(exist_ok=True)
...
for sku in ['a2a-interaction-trace','a2a-interaction-risk-report']:
    for budget,expected in [(0,None),(4,None),(5,5),(6,6),(100,6)]:
        status,q=call('/v1/quotes',{'buyer_id':'verify-v2','service_id':sku,'budget':budget})
    ...
    for name,score in [('success',98),('failure',43)]:
        payload=json.loads((ROOT/'dashboard'/f'{name}.json').read_text())
        _,q=call('/v1/quotes',{'buyer_id':'verify-v2','service_id':sku,'budget':6})
        status,order=call('/v1/orders',{'quote_id':q['quote_id'],'buyer_id':'verify-v2','service_id':sku,'amount':q['ask_price'],'idempotency_key':str(uuid.uuid4()),'input':payload})
...
status,data=call('/v1/quotes',{'buyer_id':'verify-v2','service_id':'a2a-interaction-trace','budget':6},False)
ok('No credentials rejected',status==401)
save('unauthorized.json',{'http_status':status,'response':data})
_,q=call('/v1/quotes',{'buyer_id':'verify-v2','service_id':'a2a-interaction-trace','budget':6})
status,data=call('/v1/orders',{'quote_id':q['quote_id'],'buyer_id':'verify-v2','service_id':'a2a-interaction-trace','amount':6,'idempotency_key':str(uuid.uuid4()),'input':{'events':[]}})
...
status,data=call('/v1/orders',{'quote_id':q['quote_id'],'buyer_id':'verify-v2','service_id':'a2a-interaction-trace','amount':5,'idempotency_key':str(uuid.uuid4()),'input':payload})

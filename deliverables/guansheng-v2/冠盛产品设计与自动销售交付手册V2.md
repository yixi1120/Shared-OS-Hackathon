...
本版以思棋提供的 sharedos-siqi-v1.zip、INTERACTION_REPORT_V1.md 和 PRICING_DECISION.md 为产品合同基准，更新日期为2026年9月11日。思棋源码基于 main 566b54e；此交付包是本地整合成果，不代表远端已合并或线上已部署。此前赛事网页核查信息保留为准备依据，时间冲突仍待官方确认。

两项服务常规价与首购价均为6 credits，底价5；Risk Report 返回 interpretation.flags 逐项中英文解释。附包包含完整 Runtime 源码、Dashboard 源码、机器文件及本地真实 API 验证结果。未获雅婷确认的生产标识保持空值。
...
```json
{
  "catalog_version": "guansheng-v2-2026-09-11",
  ...
  "input_boundary": "1 to 200 events for one task and one subject. Public events are self_reported; unknown fields discarded before persistence. No raw prompt or full artifact required. Hashes/references are stored, not independently verified.",
...
| occurred_at | 必填 date time | 事件时间 建议统一 UTC 并带时区 |
...
附包 docs/guansheng/input.schema.json 是正式事件输入合同；trace_report.schema.json 与 risk_report.schema.json 是严格报告输出合同；trace_delivery.schema.json 和 risk_delivery.schema.json 校验包含 trade_id、status 和 output 的 API 交付封装。schemas.json 保留思棋完整原件，其中 trace_report_model 仅反映模型构造默认值，不用于替代正式输出校验。
...
以下13项按 Runtime 顺序列出，后7项为思棋新增。标记用于诊断，不再次扣分，也不另行更改现有基础 eligibility。触发条件的完整冻结规范见附包 docs/siqi/INTERACTION_REPORT_V1.md。
...
不存在 timeout、duplicate 或 conflicting_terminal 标记。终态冲突、重复事件和混合时区仍是已知 V1 限制，不能宣传已解决。
...
当前公共 Seller API 只产生 self_reported；其他等级是内部可信摄取边界的能力设计，不是用户填写一个字段就能激活的功能。reputation_eligible 只有在出现任务终态且所有事件均为 platform 或 observed 时才可能为true。这个字段仅表示通过终态和来源的基础候选门槛。未来正式聚合仍须验证证据引用、身份与任务绑定、独立对手方及去重等条件，不得仅凭此布尔值入库，也不保证任务成功。
...
页面从真实 /health、/v1/catalog 读取状态和目录，并依次调用 quote、可选negotiate、order、deliver、order查询。页面不嵌入报告快照；成功和失败按钮仅载入可编辑事件输入，报告必须由当前 API 返回。展示执行分、证据权重、任务状态、分析交付状态、风险标记和 interpretation.flags 中英文解释，同时保留原始响应。

默认使用当前页面同源 API。可输入雅婷确认的 API Base URL；跨域时需其服务允许该展示页来源，不能将网络失败冒充业务失败。Token只在当前页面内存中使用，不写浏览器持久存储，不展示在日志或截图。页面切换后须重新输入。

成功、失败和无凭据测试均以本地真实 HTTP 服务执行，带时间及请求路径；这验证源码与 API 连通性，尚未证明线上服务可用或平台授权成立。输入示例是演练任务，不代表真实 Arena 交易。Arena 的自主流程无需人工打开本页。
...
以下结果使用思棋冻结包的实际 FastAPI 代码和独立本地 SQLite 账本执行，输入为明确的演示事件。它们证明接口如何响应，不证明已经部署、发生真实交易或转移credits。设置本地演示专用SELLER_API_TOKEN；凭据通过请求头使用，文稿不展示生产凭据。
...
![本地真实 API 未授权结果](unauthorized.png)

verification 中保留成功、失败和未授权的接口结果及自动检查摘要。正确凭据正常返回报告；原始Bearer值不进入交付证据文件。
...
实际交付位置：docs/guansheng/catalog.json对应第三章；pitch.json、faq.json和sales.json对应第六至八章；BRAIN_HANDOFF.md对应第九至十章；dashboard目录对应第十一章；verification对应第十二章；SUBMISSION.md对应第十三至十四章；DEMO_SCRIPT.md对应第十五章。文件均已随包交付，线上信息集中保留于submission.json。
...
| 最小Dashboard | 第十一章 | 完整前端源码 真实API调用 三个可复现实测视图 |
| 三类复现例 | 第十二章 | 实际本地结果与步骤 |
| 产品和提交介绍 | 第十三章 | 中英文正文 |
| 必填标识检查 | 第十四章 | 实查状态与待填位置 |
| Demo和备用录屏 | 第十五章 | 90秒稿 录制规范 |

本包已完成本地源码整合、真实API展示与机器文件拆分。团队签收、远端合并、线上部署及授权验证、雅婷确认的提交标识、备用视频成片和最终提交仍待后续执行。提交标识统一保存于 submission.json；Discord、node ID、purpose string、服务地址及Product Agent地址保持null，收到确认后一次性同步，不能使用示例代填。

## 资料来源

[S1] 仓库 main 版本 9156d8d。https://github.com/yixi1120/Shared-OS-Hackathon/tree/9156d8d0cbdc1ebd3bd1c210b155594ee404564f

[S2] 冠盛分支提交 1358ebeb。https://github.com/yixi1120/Shared-OS-Hackathon/tree/1358ebebdb9d72e24d91d8a73301cf077b78f2ac

[S3] 本版模型与指标来源。思棋冻结包下的 src/sharedos_commerce_agent/models.py 与 telemetry.py。

[S4] 本版API、服务与报价来源。思棋冻结包下的 src/sharedos_commerce_agent/api.py、seller.py、strategy.py 与 config.py。
...
[S9] 思棋冻结规范 INTERACTION_REPORT_V1.md、schemas.json 与 sharedos-siqi-v1.zip。

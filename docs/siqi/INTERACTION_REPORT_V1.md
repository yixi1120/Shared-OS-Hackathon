...
| completed | bool/是 | 存在 artifact_delivered 与 task_completed，且不存在 task_failed | 任一条件不满足则 false | 记录显示交付及完成；完成与失败同时出现时 fail closed | +40/无直接影响 |
...
缺 task_started 无直接扣分；仍可从 request_received 计算延迟。缺交付同时失去完成 40 和交付 15。缺终态无 completed，延迟奖励为 0；artifact 若存在保留 15。明确失败没有独立罚分，终态可用于延迟；完成与失败同时出现时 completed=false 并失去 40 分。争议扣 25。结构不合法事件在入口拒绝，schema_valid=false 则进入比例计算。不得用 risk flags 再扣一次分。
...
顺序为下表顺序；前六项兼容原版，思棋新增七项，集成时再加入冲突终态标记。中文/英文买方文案的机器源为 risk_rules.py。S 列指触发事实已如何进入公式；flag 自身不额外扣分。R 指现有基础门槛；未来入库规则另行判断。
...
| conflicting_terminal_task_state | 同时存在 task_completed 与 task_failed | 终态冲突 / Conflicting terminal：不判为完成，等待后续核验 | completed=false，失去40 | 拒绝 | conflicting_terminals |
...
没有 timeout 或 duplicate 的新 stage/flag；这些事实仍不能由报告凭空补造。终态冲突已有诊断标记，但该标记不裁决哪一个终态真实。

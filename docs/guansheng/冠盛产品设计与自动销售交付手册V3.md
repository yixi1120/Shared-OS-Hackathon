...
两项服务常规价与首购价均为6 credits，底价5。健康检查、目录、Schema、示例和价格说明免费；生成正式报告收费。风险目录共14项，新增 conflicting_terminal_task_state；Risk Report 返回 interpretation.flags 逐项中英文解释。冲突终态令 completed 和 reputation_eligible 均为false。Discord用户名和提交频道已经确认，生产标识继续等待雅婷确认。
...
主办方最新Discord公告要求Agent在周日晚21时至23时（北京时间）两个Round全程在线；按当前2026年日历对应9月13日。历史赛事页的时区文字与日期曾不一致，执行时以Discord最新公告和`current-round`频道为准。[S6][S7][S9]
...
文本房间可直接发送。若录制语音，成片以实际计时为准，须控制在20秒以内并保留证据边界句。
...
| 参赛Agent SharedNet seat ID | 待正式competition room发布后通过sharednet join取得 |
| 历史材料中的SharedNet node ID | 若官方仍要求则待提供；不能假定与seat ID相同 |
| SharedOS principal ID | 待提供真实principal ID |
| SharedOS Agent address | 待提供真实Agent地址 |
...
本次基于最新main完成冠盛新增材料整理、真实API展示与机器文件同步。Discord用户名和`#submission`入口已经核实；线上部署、SharedNet seat ID、SharedOS principal ID、Agent address、授权验证、备用视频成片和最终提交仍待后续执行。提交标识统一保存于 submission.json；未确认的Runtime字段保持null，收到真实值后一次性同步，不能使用示例代填。
...
[S9] AICOO Discord `#arena-support` 与最新比赛公告，2026年9月12日核对。正式competition room将在比赛开始前两小时发布；最终项目通过`#submission`论坛频道提交。
